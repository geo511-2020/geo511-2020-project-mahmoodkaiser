...
tow_reason <- data %>% group_by(`TOW DESCRIPTION`) %>% summarize(count=n())
tow_reason
Police_district <- data %>% group_by(`POLICE DISTRICT`) %>% summarize(count=n())
Police_district
```
...
m <- leaflet(new_data) %>% 
   addTiles() %>% 
   setView( lng = -78.9, lat = 42.9, zoom = 10 ) %>% 
